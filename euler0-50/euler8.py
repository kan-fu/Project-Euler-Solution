class Solution:
    def __init__(self):
        import time

        start = time.time()
        self.answer = self.solve()
        elapse = time.time() - start
        if elapse > 1:
            self.time = f"{elapse:.1f}s"
        else:
            self.time = f"{elapse*1000:.1f}ms"

    def solve(self, N=13):
        """
        Sliding window problem. Keep the window length to 13 and update the window product by multiplying arr[right] 
        and getting divided by arr[left]. Tricky part is that there is 0 in the seq, which will cause trouble 
        when moving the left pointer. Modifying 0 to -1 makes the product unable to become the maximum 
        while avoiding the divided-by-zero problem.
        """
        seq = ""
        arr = list(map(int, seq))
        ans = -1
        window_product, left = 1, 0
        for right in range(len(arr)):
            if arr[right] == 0:
                arr[right] = -1
            window_product *= arr[right]
            ans = max(ans, window_product)
            if right >= N - 1:
                # when window length is equal to N, contract the window
                window_product //= arr[left]
                left += 1

        return ans


if __name__ == "__main__":
    solver = Solution()
    print(solver.answer, solver.time)
